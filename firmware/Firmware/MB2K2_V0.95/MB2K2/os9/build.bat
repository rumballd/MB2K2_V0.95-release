...
lwasm -p nosymbolcase -f -l os9 -o periph\framdisk.bin periph\framdisk.txt > periph\framdisk.lis
...
copy /B periph\termio.bin + periph\promdisk.bin + periph\ramdisk.bin + periph\framdisk.bin + periph\fndisk.bin periph\periph.bin 
copy /B mb2k2\blinky.bin cmds\cmds.bin
copy /B FLEXNet\rexit.bin + FLEXNet\rmount.bin flexnet\flexnet.bin
