...
unsigned reset_flag = 0, irq_flag = 0;
...
int read8(int addr) {
    ...
    chan_out_word(end_a, ((addr & 0xFFFF) | BYTE_READ ));
    chan_in_word(end_a, &val);
...
int read16(int addr) {
    ...
    chan_out_word(end_a, ((addr & 0xFFFF) | WORD_READ ));
    chan_in_word(end_a, &val);
...
void write8(int addr, int data) {
    ...
    chan_out_word(end_a, ((addr & 0xFFFF) | BYTE_WRITE | ((data & 0xFF) << 16)));
}
...
void cpu_execute(chanend c_addrData) {
  ...
  end_a = c_addrData; // pick up the channel ID
 ...
 // reset 6809
  ...
  Z = 1;
  EFI = F_FLAG | I_FLAG;
  change_pc (read16 (0xfffe)); // point to reset vector
...
//        if(PC == 0xA8d3) { // debug 'breakpoint' code
//            reset_flag = 0; // dummy statement for breakpoint
//        }
